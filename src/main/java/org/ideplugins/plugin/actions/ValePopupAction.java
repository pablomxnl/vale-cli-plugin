    ...
    public void actionPerformed(@NotNull AnActionEvent actionEvent) {
        if (areSettingsValid(actionEvent)){
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                ...
                //
                if (psiFile != null) {
                    output = cliExecutor.executeValeCliOnFile(psiFile);
                    reporter.updateIssuesForFile(psiFile.getVirtualFile().getPath(), output);
                } else {
                    ...
                    if (virtualFiles != null && virtualFiles.length > 1) {
                        ...
                        output = cliExecutor.executeValeCliOnFiles(filesToCheck);
                        for (String file : filesToCheck){
                            reporter.updateIssuesForFile(file, output);
                        }
                    }
                }
            });
